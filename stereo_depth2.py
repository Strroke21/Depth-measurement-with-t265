...
try:
    ...
    while True:
        ...
        if valid:
            ...
            disparity_filtered = medianBlur(disparity, 5)  # Adjust kernel size as needed
            disparity_filtered = disparity_filtered[:, max_disp:]


            baseline_mm = 64.0

            # Calculate the depth from disparity
            #depth_map = (stereo_focal_px * T[0]) / disparity
            #depth_map = (baseline_mm * stereo_focal_px) / disparity_filtered
            depth_map = np.where(disparity_filtered <= 0, np.nan, (baseline_mm * stereo_focal_px) / disparity_filtered)

            # Convert depth from pixels to meters
            depth_map = depth_map / 1000.0  # Convert from millimeters to meters

            # Print depth values
            print(f"Depth (meters): {depth_map}")
            #print(depth_map)
